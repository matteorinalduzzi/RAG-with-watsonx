...
Then the function loads the IBM Granite Guardian 38M model, which can be easily pulled from its HuggingFace Repository ([link](https://huggingface.co/ibm-granite/granite-guardian-hap-38m)). The model take strings of text as input and compute the probability that the processed text may contain HAP language: in its basic settings, if the probability associated to a string is higher than 0.5, the string is classified as 1 (indicating that it may contain HAP language), otherwise it is classified as 0 (i.e. no HAP content detected). For the sake of our analysis, we decide to select only the chuncks of text presenting a probability higher than 0.9 but the code can be easily adapted in order to select the preferred treshold for the specific use case:

![Fig.2 - Classify HAP content with IBM Granite Guardian](./images/fig2.png)
